<script lang="ts">
    ...
    onMount(() => {
        changeSource("/ost/"+ ESCAPEGAME_KEY +"/step5.mp3")
        renderStore.set(new RenderData($t(`common.step.${ESCAPEGAME_KEY}.fifthPlace`), $t(`${ESCAPEGAME_KEY}.fifthPlace.neon.title`), $t(`${ESCAPEGAME_KEY}.fifthPlace.neon.subtitle`), Step.FIFTH_PLACE));
    })
    ...
    const validateClick = () => {
        isWaiting = true
        redirect($page.params.lang, `${ESCAPEGAME_KEY}/sixthPlace`)
    }
    onMount(() => {
...
<ModalComponent opened={showTransitionModal}
    ...
    <Grid>
        <Row>
            ...
            <Column>
                {#each $t(`${ESCAPEGAME_KEY}.fifthPlace.before.row`) as row}
                    <p>{row}</p>
                {/each}
            </Column>
        </Row>
...
<TypewriterComponent disabled={showScenario} waitReading continueButtonAction={() => showEntrance = true}>
    <div>
        <div class="container mb-2">
            ...
            {#each $t(`${ESCAPEGAME_KEY}.fifthPlace.scenario.row`) as row}
                <p>{row}</p>
            {/each}
        </div>
    </div>
...
<ModalComponent opened={showEntrance}
    ...
    <Grid>
        <Row>
            <Column>
                <ImageLoader
                        src="{base}/{ESCAPEGAME_KEY}/fifthPlace/spirit.png"
                        alt="" fadeIn={true}/>
            </Column>
            <Column>
                {#each $t(`${ESCAPEGAME_KEY}.fifthPlace.scenario.modal.1.row`) as row}
                    <p>{row}</p>
                {/each}
            </Column>
        </Row>
...
<TypewriterComponent disabled={disableGoal} waitReading continueButtonAction={() => showTrialModal = true}>
    <div>
        <div class="container mb-4">
            ...
            {#each $t(`${ESCAPEGAME_KEY}.fifthPlace.goal.row`) as row}
                <p class="mb-4">{row}</p>
            {/each}
        </div>
    </div>
...
<div style="display: flex; flex-direction: column; margin-top: 2rem; align-items: center; justify-content: center;">
    <ModalComponent opened={showTrialModal} modalHeading={$t(`${ESCAPEGAME_KEY}.fifthPlace.test.modal.1.title`)}
        ...
        <div bind:this={xElement} style="position: relative;cursor: none;"
            ...
            <ImageLoader
                    src="{base}/{ESCAPEGAME_KEY}/fifthPlace/cross.png"
                    alt="" fadeIn={true}/>
            <div tabindex="0"
                 style="position: absolute; top: 30.5%; left: 46.5%; color:rgba(254, 233, 0, 0); font-size: 1.3rem; font-weight: bold"
                 on:click={() => validateClick()}
                 on:keydown={(event) => {if (event.key === "Enter") {validateClick()}}}>X
            </div>
            <div style="position: absolute; top: 50.8%; left: 38.7%; color:rgb(254, 233, 0); font-size: 1; font-weight: bold;">
                {$t(`${ESCAPEGAME_KEY}.fifthPlace.test.modal.1.content`)}
            </div>
    ...
    {#if reopenTrialModal}
        <Button kind="secondary" on:click={() => {showTrialModal = true; reopenTrialModal= true}}>{$t(`${ESCAPEGAME_KEY}.fifthPlace.test.buttons.open.text`)}
        </Button>
    {/if}
</div>
{#if isWaiting}
